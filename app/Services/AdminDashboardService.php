...
class AdminDashboardService
{
    ...
    public function getMonthlyRevenue(int $year = null): array
    {
        ...
        // PostgreSQL menggunakan EXTRACT() untuk mengambil bulan dan tahun
        $monthlyData = PickupRequest::where('status', 'delivered')
            ->whereRaw('EXTRACT(year from delivered_at) = ?', [$year])
            ->selectRaw('EXTRACT(month from delivered_at) as month, COUNT(*) as total_orders, SUM(total_amount) as total_revenue')
            ->groupByRaw('EXTRACT(month from delivered_at)')
            ->orderByRaw('EXTRACT(month from delivered_at)')
            ->get();
